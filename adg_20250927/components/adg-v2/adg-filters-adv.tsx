...
export function buildFilterFns<T>() {
  return {
    ...
    date: (row: Row<T>, columnId: string, filterValue: any) => {
      ...
      if (!filterValue) return true;
      if (filterValue.mode === "range") {
        const from = filterValue.from ? new Date(String(filterValue.from)).getTime() : undefined;
        const to = filterValue.to ? new Date(String(filterValue.to)).getTime() : undefined;
        if (from !== undefined && d < from) return false;
        if (to !== undefined && d > to) return false;
        return true;
      } else {
        const op = filterValue.op ?? "==";
        const val = filterValue.value ? new Date(String(filterValue.value)).getTime() : undefined;
...
export function FilterButton<T>({ table, column, meta }:{ table: Table<T>; column: Column<T, unknown>; meta?: ColumnFilterMeta; }) {
  ...
  const initialDraft =
    kind === "enum" ? (current ?? []) :
    (kind === "number" || kind === "date") ? (current ?? { mode: "range" }) :
    (current ?? "");
